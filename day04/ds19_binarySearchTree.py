...
for name in groupList[1:]: # 레드벨벳부터 끝까지
    ...
    while True:
        if name < curr.data: # 왼쪽 트리로
            if curr.left == None: # 왼쪽에 트리가 구성되어있지않다면
                curr.left = node
                break # while문 탈출 
            else:
                curr = curr.left
...
while True:
    if deleteName == curr.data: # 삭제할 데이터를 찾았다면
        if curr.left == None and curr.right == None: # 리프노드일 경우(삭제 매우 쉬움)
            if parent.left == curr: # 내가 부모의 왼쪽에 있다면
                parent.left = None
            else: # 내가 부모의 오른쪽에 있다면
                parent.right = None
            ...
            del(curr)
        elif curr.left == None and curr.right != None: # 내 노드 오른쪽에 자식노드가 있다면
            if parent.left == curr:
